...
class vector {
	...
	reference			back(void) { return (*this)[this->_size - 1];};
	const_reference		back(void) const { return (*this)[this->_size - 1];};
	...
	size_type				_size;
	size_type				_capacity;
	const static size_type	_max_size  = std::numeric_limits<difference_type>::max() / (sizeof(value_type));
